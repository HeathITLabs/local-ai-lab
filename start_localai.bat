...
REM Existing database must have a verified checkpoint before updates.
set "SOLO_CONTAINER="
for /f "delims=" %%C in ('docker ps -a --filter "name=^/solo-postgres$" --format "{{.Names}}"') do set "SOLO_CONTAINER=%%C"
if errorlevel 1 (echo BACKUP_FAILED: Docker inspection failed.& exit /b 1)
if "!SOLO_CONTAINER!"=="" (
    set "SOLO_VOLUME="
    for /f "delims=" %%V in ('docker volume ls --filter "name=^localai_solo-postgres-data$" --format "{{.Name}}"') do set "SOLO_VOLUME=%%V"
    if errorlevel 1 (echo BACKUP_FAILED: volume inspection failed.& exit /b 1)
    if not "!SOLO_VOLUME!"=="" (echo BACKUP_FAILED: database volume exists without container.& exit /b 1)
    echo FIRST_RUN_NO_DATABASE: no container or volume found.
) else (
    powershell -NoProfile -NonInteractive -ExecutionPolicy Bypass -File "%LAB_DIR%\scripts\backup-local-ai.ps1" -Backup Solo
    if errorlevel 1 (echo BACKUP_FAILED: refusing update/start.& exit /b 1)
)
